...
- Do not assume the OpenClaw agent's own model or API key will be reused by Kaksha.
- Kaksha classroom generation uses Kaksha server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Kaksha server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use Kaksha:

1. **Use hosted Kaksha** (recommended for quick start) — Requires an access code from kaksha.chat. No local setup needed.
...
Use this when the user has not installed Kaksha yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Kaksha

After the user has chosen a startup mode and configured keys, start Kaksha using the chosen method, then verify the service with `GET {url}/api/health`.
